...
# PVWA Certificate Handler Module
# Handles PVWA-specific certificate configuration and generation

# Collect PVWA configuration
configure_pvwa() {
    echo -e "${CYAN}=== PVWA Certificate Configuration ===${NC}"
    echo "Configuring certificates for Password Vault Web Access (PVWA)"
    echo ""
    
    # Number of PVWA servers
    echo -e "${BLUE}PVWA Server Configuration:${NC}"
    local num_servers
    while true; do
        read -p "How many PVWA servers do you have? (1-10): " num_servers
    ...
    for ((i=1; i<=num_servers; i++)); do
        echo ""
        local fqdn=$(get_input "Enter FQDN for PVWA server $i" "fqdn" "")
        server_fqdns+=("$fqdn")
    done
    ...
    if [[ $num_servers -gt 1 ]]; then
        ...
        echo "1. Single certificate for all PVWA servers (with SAN entries)"
        echo "2. Unique certificate for each PVWA server"
    ...
    echo ""
    echo -e "${CYAN}=== PVWA Configuration Summary ===${NC}"
    ...
    # Collect additional SANs for PVWA
    echo ""
    collect_additional_sans "PVWA"
    
    # Generate certificates based on strategy
    if [[ "$cert_strategy" == "1" ]]; then
        generate_pvwa_single_cert "$lb_fqdn" "${server_fqdns[@]}"
    else
        generate_pvwa_multiple_certs "$lb_fqdn" "${server_fqdns[@]}"
    fi
}

# Generate single certificate for all PVWA servers
generate_pvwa_single_cert() {
    local lb_fqdn="$1"
    shift
    local server_fqdns=("$@")
    
    log_info "Generating single certificate for all PVWA servers..."
    
    # Create PVWA output directory
    local pvwa_dir="$OUTPUT_DIR/pvwa"
    mkdir -p "$pvwa_dir"
    
    # Copy and modify configuration file
    local config_file="$pvwa_dir/pvwa-cert.cnf"
    cp "$SCRIPT_DIR/pvwa-cert.cnf" "$config_file"
    ...
    local csr_file="$pvwa_dir/pvwa.csr"
    
    if generate_private_key "$key_file" "$KEY_LENGTH" && generate_csr "$key_file" "$config_file" "$csr_file"; then
        log_success "PVWA certificate files generated successfully"
        
        # Display file locations
        echo ""
        echo -e "${CYAN}=== Generated PVWA Files ===${NC}"
        ...
        create_pvwa_instructions "$pvwa_dir" "single"
        
        return 0
    else
        log_error "Failed to generate PVWA certificate files"
        return 1
    fi
}

# Generate unique certificates for each PVWA server
generate_pvwa_multiple_certs() {
    local lb_fqdn="$1"
    shift
    local server_fqdns=("$@")
    
    log_info "Generating unique certificates for each PVWA server..."
    ...
    for ((i=0; i<${#server_fqdns[@]}; i++)); do
        ...
        local server_dir="$pvwa_dir/server$server_num"
        
        log_info "Generating certificate for PVWA server $server_num: $server_fqdn"
        
        # Create server-specific directory
        mkdir -p "$server_dir"
        
        # Copy and modify configuration file
        local config_file="$server_dir/pvwa-server$server_num.cnf"
        cp "$SCRIPT_DIR/pvwa-cert.cnf" "$config_file"
        ...
        local csr_file="$server_dir/pvwa-server$server_num.csr"
        
        if generate_private_key "$key_file" "$KEY_LENGTH" && generate_csr "$key_file" "$config_file" "$csr_file"; then
            log_success "Certificate files generated for PVWA server $server_num"
        else
            log_error "Failed to generate certificate files for PVWA server $server_num"
            return 1
        fi
    done
    ...
    # Display summary
    echo ""
    echo -e "${CYAN}=== Generated PVWA Files Summary ===${NC}"
    for ((i=0; i<${#server_fqdns[@]}; i++)); do
        local server_num=$((i+1))
        local server_dir="$pvwa_dir/server$server_num"
        echo "Server $server_num (${server_fqdns[i]}):"
        echo "  Private Key: $server_dir/pvwa-server$server_num.key"
        echo "  CSR: $server_dir/pvwa-server$server_num.csr"
        echo "  Configuration: $server_dir/pvwa-server$server_num.cnf"
    done
    echo ""
    
    # Create instructions file
    create_pvwa_instructions "$pvwa_dir" "multiple" "${#server_fqdns[@]}"
    
    return 0
}

# Update SAN entries in configuration file
update_san_entries() {
    local config_file="$1"
    shift
    local san_entries=("$@")
    ...
    while IFS= read -r line; do
        ...
        elif [[ "$in_alt_names" == true ]] && [[ "$line" =~ ^\[.*\] ]]; then
            # End of alt_names section
            in_alt_names=false
            echo "$line"
        elif [[ "$in_alt_names" == true ]] && [[ "$line" =~ ^DNS\.|^IP\. ]]; then
            # Skip existing SAN entries
            continue
        else
            echo "$line"
        fi
...
3. CONVERT TO PFX FORMAT
   ...
   Select option 3 (Convert Signed Certificates) and choose PVWA.
   
   This will generate:
   - $pvwa_dir/pvwa.pfx (unprotected PFX file for PVWA)
...
EOF
        for ((i=1; i<=num_servers; i++)); do
            cat >> "$instructions_file" << EOF
   - $pvwa_dir/server$i/pvwa-server$i.csr
EOF
        done
...
EOF
        for ((i=1; i<=num_servers; i++)); do
            cat >> "$instructions_file" << EOF
   - $pvwa_dir/server$i/pvwa-server$i.crt
EOF
        done
...
EOF
        for ((i=1; i<=num_servers; i++)); do
            cat >> "$instructions_file" << EOF
   - $pvwa_dir/server$i/ca-chain.crt
EOF
        done
...
EOF
        for ((i=1; i<=num_servers; i++)); do
            cat >> "$instructions_file" << EOF
   - $pvwa_dir/server$i/pvwa-server$i.pfx (unprotected PFX file)
EOF
        done
    fi
